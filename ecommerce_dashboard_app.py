...
# === PAGE: CUSTOMER SEGMENTATION ===
    ...
    fig = px.bar(x=cluster_counts.index, y=cluster_counts.values, labels={'x': 'Cluster', 'y': 'User Count'},
                 title="Cluster Distribution", color=cluster_counts.values)
    st.plotly_chart(fig, use_container_width=True)
...
# === PAGE: MODEL PERFORMANCE ===
    ...
    model_data = pd.DataFrame({
        'Model': ['Random Forest', 'XGBoost', 'Neural Network', 'Naive Bayes', 'SVM'],
        'Accuracy': [0.99, 0.98, 0.94, 0.91, 0.92],
        'Recall':   [0.91, 0.88, 0.75, 0.01, 0.00],
        'F1-Score': [0.95, 0.92, 0.82, 0.02, 0.00],
        'ROC-AUC':  [0.999, 0.998, 0.89, 0.55, 0.50]
    })
...
# === PAGE: FUNNEL ANALYSIS ===
    ...
    funnel_fig = px.bar(funnel_df, x=funnel_df.index, y=['login','page_view','product_view','add_to_cart','purchase'],
                        title="Funnel Completion per Cluster")
...
# === PAGE: PRODUCT RECOMMENDATIONS ===
    ...
    sample = pd.DataFrame({
        'UserID': [1, 5, 9],
        'Top Product Recommended': ['prod_6845', 'prod_2594', 'prod_8422']
    })
    st.dataframe(sample)
